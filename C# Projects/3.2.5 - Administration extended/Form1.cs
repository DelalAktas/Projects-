...
namespace _3._2._4___Administration
{
				public partial class Form1 : Form
				{
								...
								private void updateInterface()
								{
												...
												foreach (ProjectGroup g in c.Groups)
								...
								private void searchStudents(TextBox tb)
								{
												...
												if (inpText.Length > 0)
												{
																if (!Regex.IsMatch(inpText, notNumRegex))
																{
																				int num;

																				try
																				{
																								num = Convert.ToInt32(inpText);
																				}
																				catch (Exception e) {
																								return;
																				}

																				s = c.SearchByStudentnumber(num);
																}
												}
												else
												{
